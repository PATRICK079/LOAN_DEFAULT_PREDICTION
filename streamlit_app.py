...
def load_col():
	col_name  = joblib.load("col_names.pk1")
	return col_name
...
def make_prediction(_model, _scaler, _col_name, X_pred):
	...
	num_scaled = scaler.transform(loan[:, :24])
	loan = np.hstack([num_scaled, loan[:, 24:]])
	prediction = model.predict(loan)
	if prediction == 1:
		return '(1) -> This client will not be able to pay back when given a loan'
	else:
		return '(0) -> This client would pay back when given a loan'
